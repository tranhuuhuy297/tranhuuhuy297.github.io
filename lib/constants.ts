...
export const projectsData = [
  {
    ...
    overview: 'Enterprise SaaS platform for presales and customer success, serving Mizuho (Japan\'s 3rd largest bank) and other major Japanese corporations. Features a collaborative workspace with Notion-like editing built on Lexical core.',
    highlights: [
      ...
      'Background processing via graphile-worker, Postmark for transactional emails, Slack SDK for notifications, Snowplow for analytics',
      'GCP infrastructure managed by Terraform, Kubernetes on GKE with Tanka configs and blue-green deployments (prod01/prod02)',
  ...
  },
  {
    ...
    highlights: [
      ...
      'Business logic follows Service-Finder-Updater pattern: services orchestrate, finders read, updaters write — fully decoupled from endpoint layer',
      'Async processing via SQS queues + EventBridge-triggered cron Lambdas; internal Lambda-to-Lambda calls via privileged_api_invoker bypassing API Gateway',
      'Multi-tenant architecture — MongoDB, MySQL, Redis, Elasticsearch all partitioned by organ_id for tenant isolation',
      'BigQuery analytics pipeline, DynamoDB for operational key-value storage, CloudFormation with splitStacks plugin to stay within AWS resource limits',
